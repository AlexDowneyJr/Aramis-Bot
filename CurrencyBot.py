...
async def ban(ctx, username : discord.Member, reason : str = None):
    ...
    if author == username:
        ...
        return
            ...
            await bot.say("I cannot let you do that. You ain't got enough power for it.")
            return
...
@bot.command(pass_context=True)
async def info(ctx):
    embed = discord.Embed(title="AMMUNO - The Producers clone Bot", description="The Producer who you'd fuck for your money", color=0xF00000)
...
async def insult(ctx, username : discord.Member = None):
    ...
    if username == None:
        ...
        await bot.say(username.mention + " " + random.choice(insult_thing))

bot.remove_command("help")

@bot.command()
async def help():
    embed = discord.Embed(title="AMMUNO - The Official Bot", description="The Bot that sees all, hears all and knows all. List of commands are:", color=0xF00000)
